...
- **Windows**: a `.exe` installer (built with NSIS). It installs to
  `%LOCALAPPDATA%\doomgeneric-WASM` and adds Desktop and Start Menu
  shortcuts, plus an uninstaller. It is unsigned, so Windows SmartScreen
  may warn on first run ("More info" > "Run anyway").
- **macOS**: a `.dmg` disk image containing a "DOOM (WASM)" app. Drag it to
  Applications as usual. It is unsigned and not notarized (no Apple
  Developer account behind this project), so the first launch needs
  right-click > Open, or a pass through System Settings > Privacy &
  Security > "Open Anyway".

All three are the engine plus the in-browser WAD picker only, no bundled
freeware games (keeps them small). They just open `index.html` in your
default browser, exactly like running `install.sh` yourself would; supply
your own WAD (or a freely redistributable one) from the picker.
